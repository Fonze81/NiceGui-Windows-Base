# -----------------------------------------------------------------------------
# File: tests/infrastructure/test_byte_size.py
# Purpose:
# Validate byte-size parsing behavior for infrastructure configuration values.
# Behavior:
# Covers accepted integer and string values, invalid formats, unsupported types,
# and non-positive values.
# Notes:
# These tests intentionally keep the parser behavior strict. Strings must include
# an explicit byte unit, while integer values are treated as raw byte counts.
# -----------------------------------------------------------------------------
