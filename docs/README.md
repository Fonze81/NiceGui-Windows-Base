...
7. [Logging subsystem](logging.md) — logger architecture, buffering, rotation, and maintenance guidance.
8. [Code quality with Ruff](code_quality.md) — linting, formatting, and pre-commit validation.
...
- `infrastructure/logger` owns memory-buffered startup logging, console logging, rotating file logging, and logger shutdown. See [Logging subsystem](logging.md).
...
Detailed runtime evidence, such as `sys._MEIPASS`, asset paths, port selection, splash handling, and repeated resize or move events, is kept at `DEBUG` level. For the logger package internals, see [Logging subsystem](logging.md).
