...
| Module execution    | `python -m desktop_app`           | Validate package execution through `__main__.py`  |
...
sequenceDiagram
    ...
    participant MainModule as __main__.py
...
- native mode enabled;
- reload disabled;
- execution routed through `desktop_app.__main__:run`;
- `desktop_app.app` executed with `__main__` semantics;
...
- reload watches Python files under `src`;
...
The index page displays a PNG illustration from:

```text
src\desktop_app\assets\page_image.png
```

The UI uses a shared SPA shell mounted through `ui.sub_pages`. The index page renders a centered card layout with the image, the application title, a short description, and the startup status message.

`ui/pages/index.py` resolves the image path through `resolve_asset_path(...)`, which supports both normal execution and packaged execution.
